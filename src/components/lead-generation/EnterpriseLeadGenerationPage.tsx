...
const DELIVERABLE_GROUP_META: Array<{
  ...
  { label: { sr: "Audit", en: "Audit" }, icon: ClipboardList, slice: [0, 3] },
  { label: { sr: "Authority", en: "Authority" }, icon: MessageSquareText, slice: [3, 5] },
  { label: { sr: "Demand", en: "Demand" }, icon: Send, slice: [5, 8] },
  { label: { sr: "RevOps", en: "RevOps" }, icon: BarChart3, slice: [8, 10] },
];
...
const OUTCOME_META: Array<{
  ...
  { icon: BadgeCheck, kicker: { sr: "Proces", en: "Process" } },
  { icon: MessageSquareText, kicker: { sr: "Timovi", en: "Teams" } },
  { icon: Radar, kicker: { sr: "Sales", en: "Sales" } },
  { icon: MessageCircle, kicker: { sr: "Pipeline", en: "Pipeline" } },
];

export function EnterpriseLeadGenerationPage({ locale }: Props) {
  const dictionary = getDictionary(locale);
  const routes = getRoutes(locale);
  const content = getLeadGenerationPage("enterprise");
  const path = content.paths[locale];
  const otherPages = Object.values(leadGenerationPages).filter(
    (page) => page.slug !== "enterprise",
  );
    ...
    <>
      ...
      <main className="flg-preview flg-page">
          ...
          <div className="mx-auto grid max-w-6xl gap-10 px-6 py-14 md:px-8 md:py-20 lg:grid-cols-2 lg:items-center lg:gap-14">
            <div>
              <span className="flg-preview-badge">
                {locale === "sr" ? "100+ zaposlenih" : "100+ employees"}
              </span>
            ...
            <div className="flg-preview-hero__media">
              ...
              <div className="flg-preview-hero__media-card">
                ...
                <p className="mt-1 text-lg font-bold leading-snug text-foreground">
                  {locale === "sr"
                    ? "Audit → povezan revenue sistem"
                    : "Audit → connected revenue system"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mx-auto max-w-6xl px-6 md:px-8">
            <div className="flg-fit__layout">
              <AnimateInView className="flg-fit__intro">
                ...
                <p className="flg-fit__lede">
                  {locale === "sr"
                    ? "Ako se prepoznajete — enterprise pristup ima smisla."
                    : "If this sounds like you — the enterprise approach fits."}
                </p>
              </AnimateInView>
            ...
            <AnimateInView delay={220} className="flg-fit__footer">
              ...
              <Button href={routes.contact}>
                {locale === "sr" ? "Zakažite audit" : "Book an audit"}
              </Button>
            </AnimateInView>
          </div>
        </section>
            ...
            <AnimateInView>
              <header className="flg-gap__header">
                ...
                <h2 className="flg-gap__title">
                  {locale === "sr" ? (
                    <>
                      <span className="flg-gap__title-muted">Kanali postoje. </span>
                      <span className="flg-gap__title-strong">Sistem još nije povezan.</span>
                    </>
                  ) : (
                    <>
                      <span className="flg-gap__title-muted">Channels exist. </span>
                      <span className="flg-gap__title-strong">The system still isn’t connected.</span>
                    </>
                  )}
                </h2>
              </header>
              ...
              <div className="flg-gap__board">
                ...
                <div className="flg-gap__panel flg-gap__panel--system">
                  ...
                  <ul className="flg-gap__list">
                    {(locale === "sr"
                      ? [
                          "Audit pre nove kampanje",
                          "Kanali grade zajednički pipeline",
                          "Jasan attribution u CRM-u",
                          "Sales i marketing dele istu sliku",
                        ]
                      : [
                          "Audit before another campaign",
                          "Channels build one shared pipeline",
                          "Clear attribution in the CRM",
                          "Sales and marketing share one view",
                        ]
                    ).map((item) => (
                      ...
                      <div key={group.label.sr} className="flg-get__group">
                        ...
                        <ul className="flg-get__list">
                          {items.map((item, index) => {
                            ...
                            const ItemIcon = DELIVERABLE_ICONS[itemIndex] ?? Target;
                            return (
        ...
        <section className="flg-outcome" aria-labelledby="results-heading">
          ...
          >
            <defs>
              <linearGradient id="flg-outcome-fill-ent" x1="0" y1="0" x2="0.15" y2="1">
                <stop offset="0%" stopColor="#ffcc00" stopOpacity="0.045" />
                <stop offset="50%" stopColor="#ffcc00" stopOpacity="0.015" />
                <stop offset="100%" stopColor="#ffcc00" stopOpacity="0" />
              </linearGradient>
              <linearGradient id="flg-outcome-stroke-ent" x1="0" y1="1" x2="1" y2="0">
